...
<article class="page-container">
	...
	<input type="text" style="display: none;" id="id" name="id" value="${transfer.id }"/>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="empId" id="empId" size="1">
				<c:forEach items="${workList }" var="list"> 
					<c:if test="${list.id == transfer.empId }">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="beforeDepId" id="beforeDepId" size="1">
				<c:forEach items="${departmentList }" var="list"> 
					<c:if test="${list.id == transfer.beforeDepId }">
						<option value="${list.id }" selected="selected">${list.name }</option>	
					</c:if>
					<c:if test="${list.id != transfer.beforeDepId }">
						<option value="${list.id }">${list.name }</option>	
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="laterDepId" id="laterDepId" size="1">
				<c:forEach items="${departmentList }" var="list"> 
					<c:if test="${list.id == transfer.laterDepId }">
						<option value="${list.id }" selected="selected">${list.name }</option>	
					</c:if>
					<c:if test="${list.id != transfer.laterDepId }">
						<option value="${list.id }">${list.name }</option>	
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="beforeJobId" id="beforeJobId" size="1">
				<c:forEach items="${jobList }" var="list"> 
					<c:if test="${list.id == transfer.beforeJobId }">
						<option value="${list.id }" selected="selected">${list.name }</option>	
					</c:if>
					<c:if test="${list.id != transfer.beforeJobId }">
						<option value="${list.id }">${list.name }</option>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="laterJobId" id="laterJobId" size="1">
				<c:forEach items="${jobList }" var="list"> 
					<c:if test="${list.id == transfer.laterJobId }">
						<option value="${list.id }" selected="selected">${list.name }</option>	
					</c:if>
					<c:if test="${list.id != transfer.laterJobId }">
						<option value="${list.id }">${list.name }</option>
...
<!-- 	<div class="row cl">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${transfer.reason }" placeholder="" id="reason" name="reason">
		</div>
	</div>
	...
	<div class="row cl">
		...
		<%-- <div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${transfer.result }" placeholder="" id="result" name="result">
		</div> --%>
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="result" id="result" size="1">
				<c:if test="${transfer.result == 0 }">
					<option value="0" selected="selected">未通过</option>	
					<option value="1">通过</option>	
				</c:if>
				<c:if test="${transfer.result == 1 }">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${transfer.auditor }" placeholder="" id="auditor" name="auditor">
		</div>
	</div>
...
<script type="text/javascript">
	function edit(){
		...
		if($("#auditor").val == ""){ layer.alert("请输入审核人"); return; }
			$.ajax({
				type:"POST",
				url:"${pageContext.request.contextPath}/transfer/editTransfer",
