				...
				<tr class="text-c">
					...
					<td class="f-14">
						<c:if test="${sessionScope.user.permissions == 1 }">
							<a title="编辑" href="javascript:;" onclick="role_edit('部门编辑','<%=basePath%>/basic/editRoleDisp?id=${list.id }','','510')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
							<a title="删除" href="javascript:;" onclick="deleteRole(this,${list.id })" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</c:if>
...
function deleteRole(obj,id){
	layer.confirm('确认要删除角色吗？',function(index){
		$.ajax({
			type:'POST',
			url:'${pageContext.request.contextPath}/basic/deleteRole',
			data:{"id":id},
			dataType:'json',
...
function search(){
	var key = $("#search").val();
	window.location.href="${pageContext.request.contextPath}/basic/searchRole?key=" + key; 
}
