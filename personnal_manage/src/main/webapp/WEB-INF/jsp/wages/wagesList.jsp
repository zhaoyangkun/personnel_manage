...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 员工工资管理 <span class="c-gray en">&gt;</span> 员工工资列表  <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray"> 
 		<%if(user.getpermissions() == 1){ %>
			<span class="l"> 
				<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
				<a class="btn btn-primary radius" href="javascript:;" onclick="wages_add('添加员工工资信息','<%=basePath%>/wages/wagesAddDisp','','510')"><i class="Hui-iconfont">&#xe600;</i> 添加员工工资信息</a> 
			</span>
		<%}%>
	...
	<table class="table table-border table-bordered table-hover table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="13">员工工资管理</th>
			</tr>
			<tr class="text-c">
		...
		<tbody>
			<c:forEach items="${wagesList}" var="list" varStatus="state">
				<tr class="text-c">
					...
					<td class="f-14">
						<c:if test="${sessionScope.user.permissions == 1 }">
							<a title="编辑" href="javascript:;" onclick="wages_edit('员工工资编辑','<%=basePath%>/wages/editWagesDisp?id=${list.id }','','510')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
							<a title="删除" href="javascript:;" onclick="wages_del(this,${list.id })" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</c:if>
...
/*部门-添加*/
function wages_add(title,url,w,h){
...
/*部门编辑*/
function wages_edit(title,url,id,w,h){
	var index = layer.open({
		type:2,
		title:title,
		content:url,
...
/*部门删除*/
function wages_del(obj,id){
	layer.confirm('确认要删除员工工资信息吗？',function(index){
		$.ajax({
			type:'POST',
			url:'${pageContext.request.contextPath}/wages/deleteWages',
			data:{"id":id},
			dataType:'json',
...
function search(){
	var key = $("#search").val();
	window.location.href="${pageContext.request.contextPath}/wages/search?key=" + key; 
}
