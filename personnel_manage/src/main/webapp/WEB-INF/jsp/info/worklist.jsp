    ...
    <c:forEach items="${list}" var="basic" varStatus="test">
  	...
  	<div align="center">  
        <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第  
            ${page.pageNow} 页</font> <a href="<%=basePath %>/basicInfo/showInfo?pageNow=1&key=${key}&depId=${depId}">首页</a>  
        <c:choose>  
            <c:when test="${page.pageNow - 1 > 0}">  
                <a href="<%=basePath %>/basicInfo/showInfo?pageNow=${page.pageNow - 1}&key=${key}&depId=${depId}">上一页</a>  
            </c:when>  
            <c:when test="${page.pageNow - 1 <= 0}">  
                <a href="<%=basePath %>/basicInfo/showInfo?pageNow=1&key=${key}&depId=${depId}">上一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="<%=basePath %>/basicInfo/showInfo?pageNow=${page.pageNow}&key=${key}&depId=${depId}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
                <a href="<%=basePath %>/basicInfo/showInfo?pageNow=${page.pageNow + 1}&key=${key}&depId=${depId}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
                <a href="<%=basePath %>/basicInfo/showInfo?pageNow=${page.totalPageCount}&key=${key}&depId=${depId}">下一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="<%=basePath %>/basicInfo/showInfo?pageNow=${page.pageNow}&key=${key}&depId=${depId}">尾页</a>  
            </c:when>  
            <c:otherwise>  
                <a href="<%=basePath %>/basicInfo/showInfo?pageNow=${page.totalPageCount}&key=${key}&depId=${depId}">尾页</a>  
            </c:otherwise>  
        </c:choose>  
    </div>  
...
function searchdep() {
	...
	window.location.href="/personnel_manage/basicInfo/showInfo?key="+key+"&pageNow="+${page.pageNow}+"&depId="+depId;
}

/* function job() {
	var key = document.getElementById("search").value;
	var jobId = document.getElementById("job").value;
	window.location.href="/personnel_manage/basicInfo/showInfo?key="+key+"&pageNow="+${page.pageNow}+"&depId="+depId+"&jobId="jobId;
} */
