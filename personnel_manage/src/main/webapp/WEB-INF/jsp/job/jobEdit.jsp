...
<article class="page-container">
	<form class="form form-horizontal" id="form-job-edit">
	
	<input type="text" id="id" name="id" style="display: none;" value="${job.id }"/>
	
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>职务名称：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${job.name }" placeholder="" id="name" name="name">
		</div>
	</div>
	
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>职务简介：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${job.introduce }" placeholder="" id="introduce" name="introduce">
		</div>
	</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<textarea id="remark" name="remark" cols="" rows="" class="textarea"  placeholder="说点什么...100个字符以内" dragonfly="true" onKeyUp="$.Huitextarealength(this,100)">${job.remark }</textarea>
...
<script type="text/javascript">
	function edit(){
		...
		if(name == ""){ layer.alert("请输入职务名称"); return; }
		if(introduce == ""){ layer.alert("请输入职务简介"); return; }
		if( name != "" && introduce != "" ){
			$.ajax({
				type:"POST",
				url:"${pageContext.request.contextPath}/job/editjob",
				data:$("#form-job-edit").serialize(),
				dataType:"JSON",
				async:true,
